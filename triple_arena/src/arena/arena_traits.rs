...
use crate::{
    ...
    errors::{AllocError, DirectInsertionError, NotWithinCapacityError, ReallocationError},
    traits::{Advancer, Ptr},
};
...
Something that must be added on to `NonZeroInxGenericStack`, is that `ArenaTrait::capacity` and `ArenaTrait::reallocate_min_capacity` must additionally be limited by the max value of indexes (and if the index is not linear, the capacity is left unclamped because nothing can be inserted anyways). With all `usize`, it optimizes away to be zero cost. The remaining concern is to do with `ArenaTrait::max_capacity` and `SetMaxCapacity::set_max_capacity`, and if they should be limited by max index value as well. The main useful intention with `max_capacity`, however, is for the ability to set a hard limit and be entirely independent from the specifics of what `reallocate_min_capacity` is later going to do (besides the detail where `max_capacity` can return `None`, used to indicate if this is an unlimited type that really shouldn't be used if you are going to be concerned with limits _to begin with_). We aren't usually in the situation where we want to incrementally increase the max capacity, we usually want to only have to deal with the final `reallocate_min_capacity` telling us what is realizable in the moment. We might want to set a large limit and don't want to deal with "what do we even do if set_max_capacity fails?", we would have to introduce a max max capacity and use that, and still have to deal with the existing cases. I say that we only have the error case on `set_max_capacity` for reduction cases only and that it is ideal to be infallible otherwise. Limits arising from indexes are a super OOB thing that is specially known and is specially dealt with in `capacity` and `reallocate_min_capacity` to guarantee entry availability invariants. The only actual hard argument against it is that some impls might want small internal capacity fields less than `usize`, which can't be done if `max_capacity` is supposed to exactly follow `set_max_capacity`. Really, what we would want is something like an orthogonal `achievable_capacity` that indicates what `reallocate_min_capacity` would succeed with, however if the context is using a fully dynamic type then it is practically impossible to implement without going ahead and actually attempting reallocation, and with fixed width arrays we are already having a constant limit we are usually directly using.

other notes:

There is no trait level `generation` or `set_generation` function (or at least there won't be one without an index involved), because some implementations will have generations per slot or per domain
