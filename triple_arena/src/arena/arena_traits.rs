...
Something that must be added on to `NonZeroInxGenericStack`, is that `ArenaTrait::capacity` and `ArenaTrait::max_capacity` must additionally be limited by the max value of indexes (if the index is not linear, the capacity is left unclamped because nothing can be inserted anyways). With all `usize`, it optimizes away to be zero cost. The only strange thing that shows up from this is that `max_capacity` can return `None` on small indexes, but I consider it more important as a signal that we are dealing with a dynamic type that can fail whenever from allocation problems. The limited heap backing types (which should be used anyways if you are concerned about capacity limits), will show the index limit.
