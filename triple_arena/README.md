...
`no_std` and even `no_alloc` compatible.
...
- `SurjectArena<P, T, S>` is a special kind of union-find data structure that can associate `T`
  element entries into nonhereditary sets, which we call surjects as a shorthand. Each surject has
  a common `S` shared value
- `SimpleOrdArena<P, T>` is a fusion between an ordered balanced tree and an arena. Entries can be a
