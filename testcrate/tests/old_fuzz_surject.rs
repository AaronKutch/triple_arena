        ...
        match op_inx {
            ...
            100..=104 => {
                ...
                if len != 0 {
                    ...
                    let set_len = set.len();
                    let removed = a
                        .remove_shared(set[next_inx!(rng, set_len)].p)
                        .allow()
                        .unwrap();
                    assert_eq!(removed, v);
                    generation += 1;
            ...
            250..=299 => {
                ...
                if len != 0 {
                    ...
                    if v0 == v1 {
                        assert!(a.in_same_surject(pair0.p, pair1.p).unwrap());
                    } else {
                        assert!(!a.in_same_surject(pair0.p, pair1.p).unwrap());
                    }
                } else {
                    assert!(a.in_same_surject(invalid, invalid).is_none());
                }
            }
            300..=339 => {
                // get
                if len != 0 {
                    ...
                    let set_len = set.len();
                    let pair = set[next_inx!(rng, set_len)];
                    assert_eq!(*a.get(pair.p).unwrap(), pair.k);
                } else {
            ...
            340..=349 => {
                ...
                if len != 0 {
                    ...
                    let set_len = set.len();
                    let pair = set[next_inx!(rng, set_len)];
                    assert_eq!(*a.get_shared(pair.p).unwrap(), v);
                } else {
                    assert!(a.get_shared(invalid).is_none());
                }
            }
            350..=389 => {
                // get_mut
                if len != 0 {
                    ...
                    let set_len = set.len();
                    let pair = set[next_inx!(rng, set_len)];
                    assert_eq!(*a.get_mut(pair.p).unwrap(), pair.k);
                } else {
            ...
            390..446 => {
                ...
                if len != 0 {
                    ...
                    let set_len = set.len();
                    let pair = set[next_inx!(rng, set_len)];
                    assert_eq!(*a.get_shared_mut(pair.p).unwrap(), v);
                } else {
                    assert!(a.get_shared_mut(invalid).is_none());
                }
            }
            446..=449 => {
                // get_inx_link_no_gen
            ...
            990..=996 => {
                ...
                if len != 0 {
                    ...
                    while let Some(p) = adv.advance(&a) {
                        assert_eq!(
                            iter.next().unwrap(),
                            (p, a.get(p).unwrap(), a.get_shared(p).unwrap())
                        );
                    }
                } else {
