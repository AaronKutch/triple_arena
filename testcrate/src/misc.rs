...
use crate::cdgen::TryDrop;

/// For inspecting state right before a failure
#[derive(Debug)]
pub struct Meta<T: fmt::Debug + TryDrop> {
    pub rng: StarRng,
    pub i: usize,
    pub op_inx: usize,
    pub stats: Option<T>,
...
impl<T: fmt::Debug + TryDrop> fmt::Display for Meta<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_fmt(format_args!("{self:#?}"))
    }
}

impl<T: fmt::Debug + TryDrop> Error for Meta<T> {}

impl<T: fmt::Debug + TryDrop> Meta<T> {
    ...
    pub fn test<F: FnOnce(&mut Meta<T>) -> Result<(), StackedError>>(
        ...
        let drop_res = self.stats.take().unwrap().try_drop().stack();
